 ...
 * function also sets the flags ICR, MULT and IICEN
 *
 * @parameter: void
 * @return: void
 */
...
uint8_t Read_Luminosity(uint8_t dev_addr, uint8_t reg_addr)
{
	...
	I2C0->D = (dev_addr << 1) | READ_REG; // making the LSB as 1 to read from the register
	ACK_SIGNAL
	I2C0_WAIT;
	...
	data = I2C0->D; // reading dummy value from the register to get the actual value in next read
	ACK_SIGNAL
	I2C0_WAIT;

	I2C0_M_STOP;

	data = I2C0->D; // reading value from the register and storing in the data variable
...
uint16_t Read_Temp_Hum(uint8_t dev_addr, uint8_t reg_addr)
{
	...
	I2C1->D = ((dev_addr << 1) | READ_REG); // making the LSB as 1 to read from the register
	TRANS_COM
	I2C1_WAIT;
